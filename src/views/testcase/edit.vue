<template>
  <div class="project-container" style="width: 50%;">
    ...
    >
      ...
      <el-form-item label="测试用例说明" prop="caseDesc">
...
<script>
import { getById, edit } from "@/api/case";
import { getProUser } from "@/api/user";

export default {
  created() {
    getById(this.$route.params.cid).then(response => {
      const { data } = response;
      this.ruleForm = data;
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.ruleForm.id = this.$route.params.cid;
