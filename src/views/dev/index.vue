  ...
  <div class="project-container">
    <el-table :data="tableData" style="width: 100%;cursor: pointer;" @row-click="rowClick">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click.native.stop="handleEdit(scope.row.id)">创建任务</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  created() {
    getList(this.$store.getters.token, false, 1).then(response => {
      const { data } = response;
      this.tableData = data;
    });
  },
